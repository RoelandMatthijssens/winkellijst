class ShoppingListItem < ActiveRecord::Base
  ...
  def self.add_to_shopping_list_item(shopping_list, item, amount, user)
    shopping_list_item = self.get_or_create_shopping_list_item(shopping_list, item)
  ...
  private
  def self.get_or_create_shopping_list_item(shopping_list, item)
    shopping_list_items = self.where(shopping_list:shopping_list).where(item:item)
      if shopping_list_items.empty?
        return self.create!(item: item, shopping_list: shopping_list, amount:0)
      end
